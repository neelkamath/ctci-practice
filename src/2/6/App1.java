 ...
 * <p>
 * Solution:
 * Comparing the list to a reversed copy.
 */
public final class App1 {
    ...
    private static boolean isPalindrome(final LinkedList list) {
        final var reversed = new LinkedList();
        for (var node = list.getHead(); node != null; node = node.getNext()) reversed.prepend(node.getData());
        for (
                LinkedListNode listNode = list.getHead(), reversedNode = reversed.getHead();
                listNode != null;
                listNode = listNode.getNext(), reversedNode = reversedNode.getNext()
        )
            if (listNode.getData() != reversedNode.getData()) return false;
        return true;
    }
}
